...
describe("channel-auth", () => {
  ...
  it("passes a fail-closed QQBot account shell to plugin-owned login persistence", async () => {
    const qqbotPlugin = {
      ...plugin,
      id: "qqbot",
      gateway: {},
    };
    mocks.normalizeChannelId.mockReturnValue("qqbot");
    mocks.getChannelPlugin.mockReturnValue(qqbotPlugin);
    mocks.loadConfig.mockReturnValue({ channels: { qqbot: {} } });
    mocks.resolveChannelDefaultAccountId.mockReturnValue("default");

    await runChannelLogin({ channel: "qqbot" }, runtime);
