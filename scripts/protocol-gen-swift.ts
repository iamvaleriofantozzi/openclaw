...
// Gateway payloads evolve additively within a protocol version. Keep literal
// validation for these nested models without rejecting future optional fields.
const ADDITIVE_LITERAL_STRUCTS = new Set([
  "SystemAgentChatAcknowledgementQuestion",
  "SystemAgentChatPresentation",
]);
  ...
  if (named) {
    ...
    const itemSchema = Array.isArray(schema.items)
      ? schema.items.length === 1
        ...
        : undefined
      : schema.items;
    base = `[${swiftType(itemSchema ?? { type: "Any" }, true, true)}]`;
  } else if (schema.enum) {
    base = "String";
...
function swiftStructPropertyType(
  structName: string,
  key: string,
  schema: JsonSchema,
  required: boolean,
): string {
  if (structName === "SystemAgentChatResult" && key === "question") {
    // This shipped as a dictionary before the schema gained an object union.
    // Preserve the Swift API while TypeBox continues to validate each branch.
    return required ? "[String: AnyCodable]" : "[String: AnyCodable]?";
  }
  return swiftType(schema, required, true);
}
...
function emitStructInitializerValidation(name: string): string {
  if (name === "SystemAgentChatAcknowledgementQuestion") {
    return '\n        precondition(options.count == 1, "QR acknowledgement requires exactly one option")';
  }
  if (name === "SystemAgentChatResult") {
...
function emitStructDecoderValidation(name: string): string {
  if (name !== "SystemAgentChatAcknowledgementQuestion") {
    return "";
  }
...
function emitStruct(name: string, schema: JsonSchema): string {
    ...
    lines.push(
      "\n    public init(\n" +
        ...
        Object.entries(props)
          ...
          .join("\n") +
        emitStructInitializerValidation(name) +
        "\n    }\n\n" +
        "    private enum CodingKeys: String, CodingKey {\n" +
        codingKeys.join("\n") +
        "\n    }\n\n" +
        "    public init(from decoder: Decoder) throws {\n" +
        (schema.additionalProperties === false && !ADDITIVE_LITERAL_STRUCTS.has(name)
        ...
        Object.entries(props)
          ...
          .join("\n") +
        emitStructDecoderValidation(name) +
        "\n    }\n\n" +
  ...
  for (const [key, propSchema] of Object.entries(props)) {
    ...
    const propType = swiftStructPropertyType(name, key, propSchema, required.has(key));
    lines.push(`    public let ${propName}: ${propType}`);
    ...
    "\n    public init(\n" +
      Object.entries(props)
        .map(([key, prop]) => {
          ...
          if (name === "SystemAgentChatResult" && key === "question") {
            return "        question: [String: AnyCodable]? = nil";
          }
          return `        ${swiftInitializerParam({
            name: propName,
            schema: prop,
...
function emitStructCustomCodable(
  ...
  if (name === "SystemAgentChatResult") {
    const decodedProperties = Object.entries(props).map(([key, propSchema]) => {
      const propName = swiftStoredPropertyName(name, key);
      const type = swiftStructPropertyType(name, key, propSchema, true);
      return required.has(key)
        ? `        self.${propName} = try container.decode(${type}.self, forKey: .${propName})`
        : `        self.${propName} = try container.decodeIfPresent(${type}.self, forKey: .${propName})`;
    });
    const encodedProperties = Object.keys(props).map((key) => {
      const propName = swiftStoredPropertyName(name, key);
      return required.has(key)
        ? `        try container.encode(${propName}, forKey: .${propName})`
        : `        try container.encodeIfPresent(${propName}, forKey: .${propName})`;
    });
    return (
      "\n\n    public init(from decoder: Decoder) throws {\n" +
      "        let container = try decoder.container(keyedBy: CodingKeys.self)\n" +
