...
export function resolveTwilioStatusCallbackUrl(publicWebhookUrl: string): string {
  ...
  if (
    !retryPolicies.some((policy) => {
      const normalized = policy.toLowerCase();
      return normalized === "all" || normalized === "5xx";
    })
  ) {
    retryPolicies.push("5xx");
  }
  ...
  // Delivery state is committed before acknowledgement. Twilio must retry the
